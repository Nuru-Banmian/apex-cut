...
策略字段:
  - triggers: 什么事件触发保留（damage_dealt / kill_occurred / knock_occurred / squad_wipe_occurred）
...
## 可检测的事件（视觉模型从 UI 读取）
- squad_wipe_occurred: 准星下方弹字"团灭" — 最高价值
- kill_occurred: 准星下方弹字"击杀" / 右上角人头数+1
- knock_occurred: 准星下方弹字"击倒"
- assist_occurred: 右上角助攻数+1（你参与了击杀但不是最后一枪）
- damage_dealt: 右上角伤害数字增加 ≥ 阈值（正在交火）
...
{{
    ...
    "questions": [
        {{"id": "q0", "text": "要不要保留所有击杀，还是只要多杀/团灭？"}},
        {{"id": "q1", "text": "要不要删舔包搜刮？"}}
    ]
}}
...
{{
    ...
    "message": "方案摘要",
    "segment_strategy": {{
        "triggers": ["kill_occurred", "squad_wipe_occurred"],
        "min_damage": 50,
        "padding_before": 15,
        "padding_after": 10,
        "merge_gap": 8,
...
可用触发事件（系统能从画面 UI 检测到的）：
- `damage_dealt` — 伤害数字增加（正在交火，配合 min_damage 控灵敏度）
- `kill_occurred` — 人头数+1 或 弹字"击杀"
- `knock_occurred` — 弹字"击倒"
- `assist_occurred` — 助攻数+1（参与击杀）
- `squad_wipe_occurred` — 弹字"团灭"
- `in_combat` — 以上任一发生（最宽泛，搭配 min_damage=0 等同于所有战斗）

翻译对照：
- 用户说"所有战斗""战斗片段""打架的" → triggers: ["in_combat"] 或 ["damage_dealt", "kill_occurred", "knock_occurred", "assist_occurred", "squad_wipe_occurred"]
  - 区别：`in_combat` 是最宽泛的，任何交火都保留（包括只打中几枪没击杀的）
  - 具体列 5 个可以用 min_damage 过滤掉轻微摩擦
- 用户说"击杀集锦""只要击杀""杀人镜头" → triggers: ["kill_occurred", "squad_wipe_occurred"]
- 用户说"只要团灭""只要一穿三" → triggers: ["squad_wipe_occurred"]
- 用户说"精彩操作""高光时刻" → triggers: ["kill_occurred", "squad_wipe_occurred", "knock_occurred"]
- 用户说"完整对局""整局""所有战斗从跳伞到吃鸡" → triggers: ["in_combat", "kill_occurred", "knock_occurred", "assist_occurred", "squad_wipe_occurred"] 或直接用 ["in_combat"]
...
默认权重:
- squad_wipe_occurred: 10（团灭最高价值）
- kill_occurred: 5
- knock_occurred: 4
- assist_occurred: 3（参与击杀但非最后一枪）
- damage_dealt: 1（纯交火没有击杀，价值最低）
- in_combat: 1（与 damage_dealt 同级）

用户说"只要团灭" → squad_wipe 权重提到 10 其余 0。
...
def director_chat(state: VideoEditState, messages: list[dict], answers: dict | None = None) -> dict:
    ...
    try:
        ...
        override = ""
        if force_plan:
            override = "\n\n## ⚠️⚠️⚠️ 已达提问上限（2轮）！本轮必须输出 phase=plan，禁止再提问！"
...
def _fallback_strategy() -> dict:
    """默认策略：保留所有战斗 + 排除非战斗场景."""
    return {
        "triggers": ["in_combat", "kill_occurred", "knock_occurred", "assist_occurred", "squad_wipe_occurred"],
        "min_damage": 30,
        "padding_before": 20,
        "padding_after": 20,
        "merge_gap": 8,
